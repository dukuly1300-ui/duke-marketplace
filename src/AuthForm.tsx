  ...
  return (
    <div style={{ maxWidth: 400, margin: '60px auto', padding: 24, background: 'white', borderRadius: 16, boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {message ? <p style={{ color: '#16a34a', fontSize: 14 }}>{message}</p> : null}
      ...
      <p style={{ textAlign: 'center', marginTop: 16, fontSize: 14 }}>
        {mode === 'signin' ? (
          <span>Don't have an account? <button onClick={() => { setMode('signup'); setError(null); setMessage(null) }} style={{ background: 'none', border: 'none', color: '#1e3a8a', fontWeight: 600, cursor: 'pointer' }}>Sign up</button></span>
        ) : (
          <span>Already have an account? <button onClick={() => { setMode('signin'); setError(null); setMessage(null) }} style={{ background: 'none', border: 'none', color: '#1e3a8a', fontWeight: 600, cursor: 'pointer' }}>Sign in</button></span>
        )}
      </p>
    </div>
  )
